# Fiche académie — Coconut Samui Rugby Academy

> Source : `src/config/site.ts`, `src/data/programs.ts`, `README.md` (extraits le 2026-07-18).
> En cas de doute ou de contradiction, le code du site fait foi.

## Identité

- **Nom** : Coconut Samui Rugby Academy — abrégé **CSRA**
- **Tagline** : *Grow Strong Together.*
- **Positionnement** : la première académie de rugby structurée de Koh Samui
- **Coaching** : bilingue **français / anglais** — coachs professionnels
- **Mission sociale** : chaque événement corporate finance des bourses (scholarships)
  pour mettre des enfants de l'île sur le terrain

## Contacts & canaux

- **WhatsApp** (canal principal de conversion) : **+66 63 375 3316**
  — lien : `https://wa.me/66633753316?text=<message encodé>`
- **Email** : coconutrugbyacademy@gmail.com
  — reçoit aussi les soumissions des formulaires du site (FormSubmit)
- **Instagram** : [@coconut_samui_rugby](https://www.instagram.com/coconut_samui_rugby)
- **Site** : https://coconutsamuirugby.com

## Lieux d'entraînement

1. **Koh's 33 Stadium — Lamai** (terrain principal)
2. **PSG Academy — Chaweng**

Koh Samui, province de Surat Thani, Thaïlande.

## Programmes

### 1. Kids Rugby — 4 à 10 ans (« Fun first »)

- **Format** : tag rugby uniquement, **zéro contact**, zéro pression
- **Groupes** : max 12 enfants par coach
- **Séances** : 60 minutes, hebdomadaire
- **Expérience** : aucune requise — la plupart des enfants partent de zéro
- **À apporter** : gourde, baskets ou crampons, crème solaire
- **CTA** : essai gratuit — message WhatsApp type :
  *« Hi! I'd like to book a free trial for the Kids program (ages 4-10). »*
- **Points FAQ** : sécurité (pas de plaquage ni mêlée), débutants bienvenus,
  coaching bilingue FR/EN dès le premier jour

### 2. Teens Rugby — 11 à 17 ans (« The pathway »)

- **Format** : rugby à contact, introduit **progressivement** selon l'âge et le niveau
  (technique sur boucliers avant tout contact réel ; plafond = directives World Rugby age-grade)
- **Séances** : 90 minutes, hebdomadaire
- **Pathway** : camps intensifs, premier tournoi inter-écoles de l'île, matchs contre
  équipes de passage
- **Débutants** : bienvenus — piste « skills » parallèle dans la même séance (à niveau en 4–6 semaines)
- **À apporter** : crampons, protège-dents, eau — le reste est fourni
- **CTA** : essai gratuit — message WhatsApp type :
  *« Hi! I'd like to book a free trial for the Teens program (ages 11-17). »*

### 3. Adults Touch Rugby — 18+ (« Sunset sessions »)

- **Format** : touch rugby (sans contact), équipes mixtes refaites chaque semaine
- **Public** : résidents, expats, voyageurs — tous niveaux, ambiance sociale d'abord
- **Engagement** : aucun — paiement à la séance, drop-in bienvenu même pour une semaine sur l'île
- **CTA** : *« Hi! I'd like to join an adults touch rugby session. »*

### 4. Corporate Team-Building (« Build your crew »)

- **Formats** : demi-journée (3 h) ou journée complète, options traiteur
- **Taille** : 8 à 60 personnes
- **Lieu** : notre terrain, le lieu du client, ou la plage — tout le matériel est fourni
- **Contenu** : jeux de communication, défis d'équipe, tournoi de touch commenté, débrief + remise de prix
- **Impact** : chaque événement finance des bourses pour les enfants de l'académie
- **CTA** : devis — *« Hi! I'd like a quote for a corporate team-building event. »*

## Informations NON publiées — ne jamais inventer

| Info | Statut |
|---|---|
| Tarifs (tous programmes) | `[À COMPLÉTER PAR CYRIL]` — rediriger vers WhatsApp |
| Horaires précis des séances | `[À COMPLÉTER PAR CYRIL]` — rediriger vers WhatsApp |
| Dates des camps / événements | `[À COMPLÉTER PAR CYRIL]` (`src/pages/camps-events.astro`) |
| Noms et bios des coachs | `[À COMPLÉTER PAR CYRIL]` (`src/pages/about.astro`) |

## État du site (checklist lancement, cf. README)

- [x] WhatsApp, email, Instagram, lieux — réels
- [ ] Activation FormSubmit (premier envoi + clic « Activate » dans l'email reçu)
- [ ] Domaine définitif dans `site.ts`, `astro.config.mjs`, `robots.txt`
- [ ] GA4 + Microsoft Clarity (stubs commentés dans `src/layouts/Base.astro`)
- [ ] Dates de camps, vraies photos galerie, bios coachs
- [ ] Google Business Profile + Search Console
